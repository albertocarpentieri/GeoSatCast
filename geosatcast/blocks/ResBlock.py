...
from msscast.utils import activation, normalization
...
class ResBlock3D(nn.Module):
    def __init__(
        ...
        if 'num_groups1' in norm_kwargs:
            norm_kwargs1 = {'num_groups':norm_kwargs.pop('num_groups1')}
        else:
            norm_kwargs1 = norm_kwargs
        
        if 'num_groups2' in norm_kwargs:
            norm_kwargs2 = {'num_groups':norm_kwargs.pop('num_groups2')}
        else:
            norm_kwargs2 = norm_kwargs
        self.norm1 = normalization(in_channels, norm_type=norm, **norm_kwargs1)
        self.norm2 = normalization(out_channels, norm_type=norm, **norm_kwargs2)
        if spectral_norm:
