...
from geosatcast.blocks.NAT import NATBlock2D, NATBlock3D, CrossNATBlock2D, CrossNATBlock3D
...
class CondEncoder(nn.Module):
    def __init__(
        ...
        for i in range(len(strides)):
            ...
            if stride != 1:
                sequential = True
                layers.append(conv_nd(3,in_channels[i],out_channels[i],kernel_size=stride,stride=stride))
        if sequential:
